...
class MediaWorker(QThread):
    ...
    def run(self):
        ...
        while self.running:
            if not self.media_path or (self.media_path not in ['OBS_CAMERA', 'VIRTUAL_PET', 'TERRARIA_AI'] and not self.media_path.startswith('http') and not self.media_path.startswith('STREAM:') and not os.path.exists(self.media_path)):
                time.sleep(0.1)
                continue
